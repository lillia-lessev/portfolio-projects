"""
Unit tests for the News Application REST API, newsletters and signals.
"""
...
class ArticleAPITestCase(TestCase):
    """Test cases for Article API endpoints and related behaviour."""

    def setUp(self):
        self.client = APIClient()
        self.web_client = Client()   # for ordinary page views

        self.reader = CustomUser.objects.create_user(
            username='reader1', email='reader1@test.com',
            password='password1', role='reader'
        )
        self.journalist = CustomUser.objects.create_user(
            username='journalist1', email='journalist1@test.com',
            password='password1', role='journalist'
        )
        self.other_journalist = CustomUser.objects.create_user(
            username='journalist2', email='journalist2@test.com',
            password='password1', role='journalist'
        )
        self.editor = CustomUser.objects.create_user(
            username='editor1', email='editor1@test.com',
            password='password1', role='editor'
        )
        self.publisher_user = CustomUser.objects.create_user(
            username='publisher1', email='publisher1@test.com',
            password='password1', role='publisher'
        )
        self.publisher = Publisher.objects.create(
            name='Test Publisher', owner=self.publisher_user
        )
        ...
        # Newsletters for filtering tests
    ...
    # ---------- Reader can only retrieve their subscribed content ----------
    def test_reader_subscribed_endpoint_returns_only_subscribed(self):
    ...
    def test_newsletter_creation_by_journalist(self):
    ...
    def test_newsletter_belongs_to_journalist(self):
        newsletter = Newsletter.objects.create(
            title='My Newsletter', author=self.journalist
        )
        self.assertEqual(newsletter.author.role, 'journalist')
        self.assertIn(newsletter, self.journalist.newsletters.all())

    # ---------- Newsletter list filtering by role ----------
    def test_reader_sees_only_subscribed_newsletters(self):
        """Reader only sees newsletters from subscribed journalists/publishers."""
    ...
    def test_reader_with_no_subscriptions_sees_no_newsletters(self):
        """Reader with zero subscriptions sees an empty list."""
        empty_reader = CustomUser.objects.create_user(
            username='empty_reader', email='empty@test.com',
            password='password1', role='reader'
        )
        self.web_client.login(username='empty_reader', password='password1')
        response = self.web_client.get(reverse('newsletter-list'))
        self.assertEqual(response.status_code, 200)
        content = response.content.decode()
    ...
    # ---------- Signal logic (mocked email + POST) ----------
